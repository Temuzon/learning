...
const KNOWN_DYNAMIC_SECTIONS = ["ebootux", "getux", "plantitux", "movitux"];

function renderCardsLoaders() {
  KNOWN_DYNAMIC_SECTIONS.forEach((section) => {
    const container = sectionContainer(section);
    if (!container) return;
    container.innerHTML = '<div class="loader" aria-label="Cargando cards"></div>';
  });
}

function clearCardsLoaders() {
  KNOWN_DYNAMIC_SECTIONS.forEach((section) => {
    const container = sectionContainer(section);
    if (!container) return;
    const onlyLoader = container.children.length === 1 && container.firstElementChild?.classList.contains("loader");
    if (onlyLoader) container.innerHTML = "";
  });
}
